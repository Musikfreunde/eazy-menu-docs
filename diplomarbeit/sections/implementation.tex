...
Bei so einer Authentifizierung spricht mman über \textit{Single Core Authentifizierung}. Dabei hat man nur den Benutzer und das 
...
Alle Android Projekte haben eine \textit{AndroidManifest.xml} Datei im Root-Verzeichnis. Diese wird benötigt, da sie  
...
Dazu kommen noch Spezifikationen gegenüber der Hardware, was soviel bedeutet wie die Auswahl von Smartphones oder Tablets
...
Die Konvention für Attribute ist, dass vor jedem Attribut \textit{android:} davor zu stehen hat. Attribute sind aber 
...
\subsection{Testing}
Beim Erstellen von einem Android Projekt, werden noch zwei Packages automatisch dazu erstellt, welche vorgefertigte Tests beinhalten. 
...
Für die Login und somit Anfangsansicht, wurde ein simples Design ausgewählt. Wie in der Abbildung zu sehen ist, wird das Logo von
EazyMenu gezeigt. Das Bild wurde mittels generierter Kotlin Klasse \textit{R} in der Ansicht angezeigt.
\begin{lstlisting}
    Row(){
                Image(
...
Ein User  ist vorhanden wenn ein Accesstoken im Response vorhanden ist.
...
Die Methode beinhaltet den Filteralgorhytmus von Menüs für einen Tag. Es wird erstmals überprüft ob das jetzige Datum vor dem Datum des Menüs ist
...
Damit man Gson verwenden kann, benötigt man die Abhängigkeit com.google.code.gson:gson:2.8.9.
Mit dem collectionType wird definiert in was für ein Typ das JSON-Objekt umgewandelt werden soll.
Abschließend wird vom Responsebody eine Collection von allen mitgegebenen Bestellungen erstellt und gespeichert.
...
Jetpack Compose bietet programmieren viele Möglichkeiten der App ein benutzerfreundliches Erscheinungsbild zu verleihen.
